...
struct __attribute__((packed)) USBGetStateData { // 63
    ...
    /* 9.3*/ uint8_t UNK1             : 1;
    /* 9.4*/ uint8_t ButtonLeftFunction  : 1;
    /* 9.5*/ uint8_t ButtonRightFunction : 1;
    /* 9.6*/ uint8_t ButtonLeftPaddle    : 1;
    /* 9.7*/ uint8_t ButtonRightPaddle   : 1;
    /*10 */ uint8_t UNK2;
    /*11 */ uint32_t UNK_COUNTER;
    ...
    /*31 */ int8_t Temperature;
    /*32 */ TouchData Touch;
    /*41.0*/ uint8_t TriggerRightStopLocation : 4;
    /*41.4*/ uint8_t TriggerRightStatus       : 4;
    /*42.0*/ uint8_t TriggerLeftStopLocation  : 4;
    /*42.4*/ uint8_t TriggerLeftStatus        : 4;
    /*43 */ uint32_t HostTimestamp;
    /*47.0*/ uint8_t TriggerRightEffect : 4;
    /*47.4*/ uint8_t TriggerLeftEffect  : 4;
    /*48 */ uint32_t DeviceTimeStamp;
    /*52.0*/ uint8_t PowerPercent : 4;
    /*52.4*/ PowerState Power     : 4;
    /*53.0*/ uint8_t PluggedHeadphones  : 1;
    /*53.1*/ uint8_t PluggedMic         : 1;
    /*53.2*/ uint8_t MicMuted           : 1;
    /*53.3*/ uint8_t PluggedUsbData     : 1;
    /*53.4*/ uint8_t PluggedUsbPower    : 1;
    /*53.5*/ uint8_t UsbPowerOnBT       : 1;
    /*53.6*/ uint8_t DockDetect         : 1;
    /*53.7*/ uint8_t PluggedUnk         : 1;
    /*54.0*/ uint8_t PluggedExternalMic : 1;
    /*54.1*/ uint8_t HapticLowPassFilter: 1;
    /*54.2*/ uint8_t PluggedUnk3        : 6;
    /*55 */ uint8_t AesCmac[8];
};

// ── DS5 USB Output Report 0x02 payload ──────────────────────────────────────
struct __attribute__((packed)) SetStateData { // 47
    /* 0.0 */ uint8_t EnableRumbleEmulation        : 1; // valid_flag0
    ...
    /* 1.0 */ uint8_t AllowMuteLight                : 1; // valid_flag1
    /* 1.1 */ uint8_t AllowAudioMute                : 1;
    /* 1.2 */ uint8_t AllowLedColor                 : 1;
    /* 1.3 */ uint8_t ResetLights                   : 1;
    ...
    /* 2.0 */ uint8_t EnableImprovedRumbleEmulation : 1; // valid_flag2
    /* 2.1 */ uint8_t AllowColorLightFadeAnimation  : 1;
    /* 2.2 */ uint8_t AllowLightBrightnessChange    : 1;
    /* 2.3 */ uint8_t UNKBITC                       : 1;
    /* 2.4 */ uint8_t UseRumbleNotHapticsLF         : 1;
    /* 2.5 */ uint8_t AllowLedBlink                 : 1;
    /* 2.6 */ uint8_t UNKBITE                       : 1;
    /* 2.7 */ uint8_t UNKBITF                       : 1;
    /* 3  */ uint8_t RumbleEmulationRight;
    /* 4  */ uint8_t RumbleEmulationLeft;
    /* 5  */ uint8_t VolumeHeadphones;
    /* 6  */ uint8_t VolumeSpeaker;
    /* 7  */ uint8_t VolumeMic;
    /* 8.0*/ uint8_t AudioMusicGain  : 4;
    /* 8.4*/ uint8_t AudioJackType   : 4;
    /* 9  */ uint8_t MuteLightMode;
    /* 10 */ uint8_t RightTriggerFFB[11];
    /* 21 */ uint8_t LeftTriggerFFB[11];
    /* 32 */ uint32_t HostTimestamp;
    /* 36.0*/ uint8_t MotorPowerLevel  : 4;
    /* 36.4*/ uint8_t AudioControl2    : 4;
    /* 37 */ uint8_t HapticLowPassFilter;
    /* 38 */ uint8_t UNKBYTE2;
    /* 39 */ uint8_t LightFadeAnimation;
    /* 40 */ uint8_t LightBrightness;
    /* 41 */ uint8_t PlayerIndicators;
    /* 42 */ uint8_t LedBlink;
    /* 43 */ uint8_t UNKBYTE3;
    /* 44 */ uint8_t LedRed;
    /* 45 */ uint8_t LedGreen;
